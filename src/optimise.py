...
class SenateOptimizer:
    ...
    def smart_prune(self, target_sparsity=0.5):
        """Redistribute weak weights to strong weights"""
            ...
            for param_name, param in state_dict.items():
                ...
                for row_idx in range(weight.shape[0]):
                    ...
                    for wi in weak_idx:
                        ...
                        row[wi] = 0
                        senator_redistributed += 1
                
                param.data = weight.to(param.dtype)
            
            total_redistributed += senator_redistributed
    ...
    def precision_prune_safe(self, significance=2):
        """Merge insignificant weights to significant ones in same row"""
    ...
    def progressive_bit_reduce(self, timeout_minutes=25, margin=0.1):
        """Reduce precision of less important weights"""
            ...
            for param_name, param in state_dict.items():
                ...
                for row_idx in range(weight.shape[0]):
                    ...
                    for col_idx in range(len(row)):
                        ...
                        # Check if 6-bit is close enough
    ...
    def quantize_senators(self):
        """Quantize each senator to 8-bit"""
        print(f"\n🔧 STAGE 4: 8-BIT QUANTIZE")
        ...
        for senator_id, data in self.senators.items():
            ...
            for param_name, param in state_dict.items():
                if isinstance(param, torch.Tensor) and param.dtype == torch.float32:
                    # Simple min-max quantization to 8-bit
                    if param.numel() > 100 and param.abs().max() > 0:
                        state_dict[param_name] = param.half()  # float16 as safe alternative
                        quantized_count += 1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python optimize.py <bundle_id> [target_sparsity]")
        sys.exit(1)
    
    bundle_id = int(sys.argv[1])
    target_sparsity = float(sys.argv[2]) if len(sys.argv) > 2 else 0.5
    ...
    optimizer.optimize(target_sparsity=target_sparsity)
    optimizer.save()
